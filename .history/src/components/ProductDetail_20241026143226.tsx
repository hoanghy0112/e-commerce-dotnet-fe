...
export default function ProductDetail({ productId }: ProductDetailProps) {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => 
      prevIndex === pimages.length - 1 ? 0 : prevIndex + 1
    );
  };
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex flex-wrap md:flex-nowrap md:space-x-6">
        ...
        <button
          onClick={handlePrev}
          className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-black/50 text-white px-4 py-2 rounded-full"
        >
          ‹
        </button>
        <button
          onClick={handleNext}
          className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-black/50 text-white px-4 py-2 rounded-full"
        >
          ›
        </button>
      </div>
