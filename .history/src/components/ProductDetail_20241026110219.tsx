...
export default function ProductDetail({ productId }: ProductDetailProps) {
  ...
  function decreaseQuantity(event: MouseEvent<HTMLButtonElement, MouseEvent>): void {
    throw new Error("Function not implemented.");
  }

  function setSelectedColor(color: any): void {
    throw new Error("Function not implemented.");
  }

  function setSelectedOption(option: any): void {
    throw new Error("Function not implemented.");
  }
        ...
        {product.storage.map((option) => (
          <button
            ...
            className={`border px-4 py-2 rounded-lg focus:outline-none ${
              selectedOption === option ? 'border-black bg-gray-200' : 'bg-white'
            }`}
        ...
        {product.colors.map((color) => (
          <button
            ...
            className={`flex items-center gap-2 border px-4 py-2 rounded-lg focus:outline-none ${
              selectedColor === color ? 'border-black bg-gray-200' : 'bg-white'
            }`}
