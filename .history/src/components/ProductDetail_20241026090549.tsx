    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-10">
        <div className="bg-white p-6 rounded-lg shadow border border-gray-300">
          ...
          <button className="border px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200">View detailed</button>
        </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {product.specifications.map((spec: string, index: number) => (
              <div key={index} className="flex justify-between text-gray-700">
                <span className="font-semibold text-gray-800">{spec.split(":")[0]}</span>
                <span>{spec.split(":")[1]}</span>
              </div>
            ))}
          </div>
          <button className="mt-4 border px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200">View detailed</button>
        </div>
      </div>
