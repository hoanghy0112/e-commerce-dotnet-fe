...
export default function ProductDetail({ productId }: ProductDetailProps) {
  ...
  <div className="bg-gray-50 p-6 rounded-lg shadow-lg border border-gray-200">
    <h2 className="text-2xl font-semibold mb-4 text-gray-900">Product Details</h2>
    <p className="text-gray-600 mb-4">{product.description}</p>
    <button className="border px-4 py-2 rounded-lg bg-white hover:bg-gray-100 text-gray-800">
      View detailed
    </button>
  </div>
  ...
  <div className="p-6 rounded-lg shadow-lg border border-gray-200">
    ...
    <div className="grid grid-cols-1 gap-y-2">
      {product.specifications.map((spec: string, index) => (
        <div key={index} className="flex">
          <span className="font-semibold text-gray-800 w-1/3">
            {spec.split(":")[0]}
          </span>
          <span className="text-gray-600 w-2/3">{spec.split(":")[1]}</span>
        </div>
      ))}
    </div>
  </div>
  ...
  {/* Bottom Button */}
  <button className="mt-4 border px-8 py-3 rounded-lg bg-white hover:bg-gray-900 text-gray-900 text-sm font-medium">
    View detailed
  </button>
</div>
