          ...
          <div className="grid grid-cols-1 gap-4">
            {product.specifications.map((spec: string, index: number) => (
              <div key={index} className="flex flex-col text-gray-700">
                <span className="font-semibold text-gray-800 mb-1">{spec.split(":")[0]}</span>
                <span className="ml-4">{spec.split(":")[1]}</span>
              </div>
            ))}
          </div>
          <button className="mt-4 border px-4 py-2 rounded-lg bg-white hover:bg-gray-200">View detailed</button>
        </div>
      </div>
