    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mt-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {product.similarProducts.map((similarProduct: any) => (
            <div key={similarProduct.id} className="bg-white p-4 rounded-lg shadow">
              <img src={similarProduct.image} alt={similarProduct.name} className="w-full h-auto mb-2" />
              <p className="font-semibold">{similarProduct.name}</p>
              <p>{similarProduct.price.toLocaleString()} đ</p>
            </div>
          ))}
        </div>
      </div>
